...
//=====================================================================
	...
	MVert* PolyMesh::addVertex(double x, double y, double z, double u, double v)
	{
		MVert* w_ptr = poolV.Request();
		new(w_ptr) MVert(x, y, z, u, v);
		w_ptr->set_index(vertices_.size());
		vertices_.push_back(w_ptr);
		return w_ptr;
	}

	MVert* PolyMesh::addVertex(const Eigen::Vector3d& point,const Eigen::Vector2d& uv)
	{
		MVert* w_ptr = poolV.Request();
		new(w_ptr) MVert(point.x(), point.y(), point.z(), uv.x(), uv.y());
		w_ptr->set_index(vertices_.size());
		vertices_.push_back(w_ptr);
		return w_ptr;
	}
...
// Mesh Based Method
//=====================================================================
	double PolyMesh::closestPoint(const Eigen::Vector3d& p, MVert* close_v)
	{
		size_t v_size = vertices_.size();
		double c_dis = 1e6;
		MVert* cv = nullptr;
		for (size_t i = 0; i < v_size; i++)
		{
			Eigen::Vector3d vp = vertices_[i]->position();
	...
	MVert* PolyMesh::closestPoint(const Eigen::Vector3d& p)
	{
		MVert* cv = nullptr;
...
//=====================================================================
	MVert* PolyMesh::splitEdgeMakeVertex(MEdge* edge)
	{
		...
		Eigen::Vector3d p = v0->position()* sp + v1->position()* (1 - sp);
	...
	MEdge* PolyMesh::splitQuadrilateralTriangle(MPolyFace* face, bool isqulaty)
	{
		...
		Eigen::Vector3d vec0 = he0->tangent();
		Eigen::Vector3d vec1 = he0->tangent();
		Eigen::Vector3d vec2 = he0->tangent();
		Eigen::Vector3d vec3 = he0->tangent();
	...
	bool PolyMesh::weldVertex(MVert* v_ori, MVert* v_tar, double tolerance)
	{
		...
		for (size_t i = 0; i < v0_he_list.size(); i++)
		{
			...
			Eigen::Vector3d p0 = v0_he->toVertex()->position();
			for (size_t j = 0; j < v1_he_list.size(); j++)
			{
				MHalfedge* v1_he = v1_he_list[i];
				Eigen::Vector3d p1 = v1_he->toVertex()->position();
	...
	bool PolyMesh::weldEdge(MEdge* e0, MEdge* e1, double tolerance)
	{
		...
		Eigen::Vector3d h0_target = he00->tangent();
		Eigen::Vector3d h1_tagert = he10->tangent();
	...
	bool PolyMesh::separateSingleVert(MVert* vert, std::vector<MVert*>& new_vert_list)
	{
		...
		Eigen::Vector3d vp = vert->position();
...
//=====================================================================
	...
	MVert* PolyMesh::splitEdgeTriangle(MEdge* edge)
	{
		...
		MVert* vt = newVertex();
		Eigen::Vector3d p0 = v0->position(); Eigen::Vector3d p1 = v1->position();
...
//=====================================================================
	...
	Eigen::Vector3d PolyMesh::calculatFaceCenter(MPolyFace* face)
	{
		if (face != nullptr) return face->getFaceCenter();
		else return Eigen::Vector3d(0, 0, 0);
	}

	Eigen::Vector3d PolyMesh::calculatEdgeCenter(MEdge* edge)
	{
